...
impl Request {
    pub fn new(id: Uuid, issue_type: String, description: String, tenant_id: Uuid) -> Self {
...
pub fn send_request(request_dto: RequestDto, sessions: &SessionStore) -> Result<String, String> {
    ...
    let tenant_uuid = {
        ...
        lock.get(&session_id).cloned().ok_or("Invalid session id")?
    };
    ...
    let request = Request::new(id, issue_type, desc, tenant_uuid);
