...
pub fn send_notice(notice_dto: NoticeDto, sessions: &SessionStore) -> 
Result<String, String> {
    ...
    let session_id = notice_dto.session_id
    ...
    let date = NaiveDate::
        parse_from_str(&date_as_string, "%Y-%m-%d")
    ...
    if date < today + notice_period {
        return Err("Vacation date should be at least 30 days from today"
            .to_string());
    }
